@admin
<x-admin-layout>
    <h1 class="font-bold text-2xl mb-3">Update Post</h1>
    <div class="flex justify-center">
        <div class="p-5 shadow-lg rounded-md bg-white w-full max-w-2xl">
            <form action="{{ route('editPost', $post->id) }}" method="POST" class="grid grid-cols-4 gap-5"
                ...
                <div class="col-span-4">
                    ...
                    <textarea type="text" placeholder="Enter content" name="content"
                        class="bg-gray-100 p-2 mt-2 rounded-md w-full active:outline-2 active outline-amber-300">{{ $post->content }}
                    </textarea>
                    @error('content')
                        <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                    @enderror
...
@master
<x-master-layout>
        <h1 class="font-bold text-2xl mb-3">Post Update</h1>
    <div class="flex justify-center">
        <div class="p-5 shadow-lg rounded-md bg-white w-full max-w-2xl">
            <form action="{{ route('editPost', $post->id) }}" method="POST" class="grid grid-cols-4 gap-5"
                enctype="multipart/form-data">
                ...
                <div class="col-span-4">
                    <label for="content" class="font-semibold">Content</label>
                    <textarea type="text" placeholder="Enter content" name="content"
                        class="bg-gray-100 p-2 mt-2 rounded-md w-full active:outline-2 active outline-amber-300">{{ $post->content }}</textarea>
                </div>
